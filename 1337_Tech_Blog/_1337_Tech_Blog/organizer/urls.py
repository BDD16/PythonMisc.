...
from .views import homepage, tag_detail, tag_list, tag_create, tasking_create, tasking_detail, TagCreate, TaskingCreate, \
    UploadFile, TaskingList, TagList, TaskingUpdate, SuccessView, DownloadImageList, \
    DownloadTheGoodsView, get_progress, MusicPlayerList  # , task_status
...
urlpatterns = [
                  ...
                  url(r'^viewmusic/$',
                      login_required(MusicPlayerList.as_view()),
                      name='organizer_music_view'),
                  url('^download/media/photos/(?P<pk>\d+)$',
                      login_required(DownloadTheGoodsView.as_view()),
                      {'document_root': settings.MEDIA_ROOT}),
                  url(r'^(?P<task_id>[\w-]+)/$', get_progress, name='task_status'),
